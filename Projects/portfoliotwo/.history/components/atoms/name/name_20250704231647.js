  ...
  useLayoutEffect(() => {
    if (isHomePage) {
      ...
      const animationContext = gsap.context(() => {
        ...
        const playForward = () => {
          gsap.timeline()
            ...
            .to(lettersToHide, {
                ...
                padding: 0,
                margin: 0,
                minWidth: 0, // Ensure all space-related properties are targeted
                stagger: 0.02,
                duration: 0.4,
                ease: "power2.in"
            }, 0);
        ...
        // --- BACKWARD ANIMATION (The Restore) ---
        const playBackward = () => {
          const tl = gsap.timeline();

          // Animate font size back to its original, measured size
          tl.to(h1, { fontSize: originalFontSize, duration: 0.5, ease: "power2.inOut" });

          // At the SAME TIME, restore the letters in the correct sequence
          tl.add(() => {
            // STEP 1: INSTANTLY restore the width using `gsap.set`.
            // This immediately makes space and pushes "fok" back to the right place.
            // The letters are still invisible (`opacity: 0`).
            ...
            // STEP 2: Now that they have space, animate their opacity back to 1.
            gsap.to(lettersToHide, {
                opacity: 1,
                duration: 0.3,
                stagger: {
                    each: 0.02,
                    from: "end" // Stagger in from the right for a natural feel
                }
            });
          }, "<"); // "<" ensures this starts at the same time as the font-size tween
        };
