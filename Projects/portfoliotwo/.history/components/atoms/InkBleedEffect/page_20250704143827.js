...
const InkBleedEffect = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const texture = textureLoader.load('/ink-blot-noise.jpg');
  ...
  return (
    <div ref={containerRef} className="ink-bleed-container">
      <canvas ref={canvasRef} className="ink-bleed-canvas" />
      <div className="content-on-top">{children}</div>
    </div>
  );
};
