...
export default function Header({
    ...
    useEffect(() => {
        ...
        router.events.on('routeChangeComplete', () => setIsExiting(false));
        router.events.on('routeChangeError', () => setIsExiting(false));

        return () => {
            router.events.off('routeChangeStart', handleRouteChange);
            router.events.off('routeChangeComplete', () => setIsExiting(false));
            router.events.off('routeChangeError', () => setIsExiting(false));
        };
    }, [router]);
    ...
    const handleClick = (href) => {
        if (router.pathname !== href) {
            ...
            setTimeout(() => {
                router.push(href);
            }, 500); // Ensure this matches animation duration
        }
    };
